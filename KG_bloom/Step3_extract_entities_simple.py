...
import csv
import re
import torch  # 新增：用于GPU/CPU智能判断
from transformers import AutoTokenizer, AutoModelForTokenClassification, pipeline
# 如果你本地没有pipeline_config.py，注释掉下面两行，改用手动配置路径
from pipeline_config import STEP2_SENT_TSV, STEP3_ENT_TSV

# ========= 解决网络问题：配置Hugging Face国内镜像（核心修改） =========
os.environ['HF_ENDPOINT'] = 'https://hf-mirror.com'  # 国内镜像源，避免连接超时
os.environ['HF_HUB_CACHE'] = './huggingface_cache'  # 模型缓存路径，避免重复下载

# ========= 路径配置（二选一：用配置文件 / 手动指定） =========
# 方式1：用配置文件（如果有pipeline_config.py）
sent_tsv_path = str(STEP2_SENT_TSV)
output_entity_path = str(STEP3_ENT_TSV)

# 方式2：手动指定路径（推荐，避免配置文件依赖）
...
# 智能判断GPU/CPU（避免无GPU时报错）
device = 0 if (torch.cuda.is_available() and 0 >= 0) else -1
...
def run_ner(sentences):
    ...
    ner_pipe = pipeline(
        ...
        device=device,  # 自动适配GPU/CPU
    )
    print(f"✅ 模型加载完成（使用设备：{'GPU' if device >=0 else 'CPU'}）！开始抽取实体...\n")
